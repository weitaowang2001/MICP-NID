...
Date: 2024/06/26
Author: Tong Xu
Log the upper bound and lower bound of the micodag method.
'''
...
def optimization(input):
    ...
    def logarithmic_callback(model, where):
        ...
        if where == gp.GRB.Callback.MIP:
            ...
            if model._obj != cur_obj or model._bd != cur_bd:
                model._obj = cur_obj
                model._bd = cur_bd
                model._data.append([timeit.default_timer() - start, cur_obj, cur_bd])
    ...
    run_time = end - start
    RGAP = m.MIPGAP
    # pd.DataFrame(m._data, columns=['time', 'upper', 'lower'])
...
if __name__ == '__main__':
    ...
    for dataset in ['8galaxy']:
        ...
        print(df)
        # df.to_csv('MICP_NID_results_non-Gaussian4.csv', index=False)
