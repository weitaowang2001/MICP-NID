...
def Optimization(Input):  # M: number of node N:number of samples
    ...
    if est == 'true':
        ...
        for i in range(len(moral)):
            List_edges.append((int(moral.values[i][0]) - 1, int(moral.values[i][1]) - 1))
            List_edges.append((int(moral.values[i][1]) - 1, int(moral.values[i][0]) - 1))
    else:
        # filename = "mgest_PearsonCorEst.txt"
        # name = "%s/%s" % (dataset_directory, filename)
        # M_edges = np.loadtxt(name, delimiter=",")
        List_edges = list(zip(*np.where(mgest == 1)))
    ...
    for i in J:  # i is not sample here. Note q is m by m
        for j in J:
            Obj4 += (quicksum(Beta[l, j] * q[i, l] for l in G_moral.neighbors(j))) * (
                quicksum(Beta[l, j] * q[i, l] for l in G_moral.neighbors(j)))
    ...
    Jacard_similarity_IP = float(len(set(Predicted_DAG).intersection(G_Original.edges()))) / (
            len(set(Predicted_DAG)) + len(set(G_Original.edges())) - len(
        set(Predicted_DAG).intersection(G_Original.edges())))
    ...
    sl_TO = [dataset, est, v, n, float(format(Lamda, '.2f')), float(format(pmu, '.3f')), graph_type,
             len(List_Original_edges),
             len(moral), m.NumConstrs, m.NumVars, m.NumBinVars, int(m.NodeCount),
    ...
    s2_TO = [dataset, est, v, n, float(format(Lamda, '.2f')), float(format(pmu, '.3f')), graph_type,
             len(List_Original_edges),
             len(moral), m.NumConstrs, m.NumVars, m.NumBinVars, "NodeCount:", int(m.NodeCount),
    ...
    all_results = [dataset, v, n, kk, End - start, m.MIPGap, SHD_cpdag, SHDs, TRP, FPR]
    ##########################################################################################
    ################################## LP Model & Result #####################################
    ##########################################################################################
    ...
    current_dir = os.path.join(current_directory, dataset)
...
for i in range(11,12):
    ...
    for lamda in [12*np.log(m[i])]:  # 5 log(m) it used to be 6.214*2 in the old paper
        for k in range(21, 31):
            for estG in ["true"]:  # trur, corest
                result_i = Optimization([Dataset, estG, k, lamda])
                print(result_i)
                results.append(result_i)
                # results.append(Optimization([Dataset, estG, k, lamda]))
    df = pd.DataFrame(results)
    # print(df)
    # dataset, v, n, kk, End - start, m.MIPGap, SHD_cpdag, SHDs, TRP, FPR]
    df = pd.DataFrame(results, columns=['network', 'm', 'n', 'k', 'Time', 'RGAP', 'd_cpdag', 'SHDs', 'TPR', 'FPR'])
    df.to_csv('./experiment results/comparison to other benchmarks with non-Gaussian errors/1-30/MISOCP_results_non-Gaussian1_30.csv', index=False)
#
