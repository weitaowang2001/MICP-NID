...
def optimization(input):
    ...
    n, p = data.shape
    l = input[4] # sparsity penalty parameter
    ...
    return run_time, RGAP, SHD_cpdag, SHDs, TPR, FPR, bic


if __name__ == '__main__':
    # print(optimization([10, 100, 4, 1]))

    lambdas = [np.log(10)/100 * i for i in range(1, 16)]
    # lambdas = [np.log(20)]
    results = []
    ...
    print(results_df)
    print(lambdas[np.argmin(results_df.BIC)] / np.log(10)*100)

    # results_df.to_csv("./alpha_results/MISOCP_NID_OA_alpha_results_4log(m)n_new.csv", index=False)
