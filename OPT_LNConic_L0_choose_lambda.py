...
# ----------------------- Constructing Original True DAG ----------------------------
    ...
    bic = N * np.trace((np.eye(M) - B) @ np.transpose(np.eye(M) - B) @ np.matmul(x.T, x) / N) + np.count_nonzero(
        B) * np.log(N)
    ...
    sl_TO=[v, n, kk, float(format(Lamda, '.2f')), float(format(pmu, '.3f')),
    ...
    s2_TO=[v, n, kk, float(format(Lamda, '.2f')), float(format(pmu, '.3f')),
           len(List_Original_edges),
    ...
    alpha_results = [v, n, kk, Lamda, End - start, m.MIPGap, SHD_cpdag, SHDs, TRP, FPR, bic]
...
if __name__ == "__main__":
    ...
    lambdas = [np.log(6)*i for i in range(1,16)]
    results = []
    for lambda_i in lambdas:
            Input = ['1dsep', 'true', 1, lambda_i]
            results.append(Optimization(Input))
    names = ["m", "n", "k", "lambda", "Time", "RGAP", "d_cpdag", "SHDs", "TPR", "FPR", "BIC"]
    results_df = pd.DataFrame(results, columns=names)
    print(results_df[['d_cpdag', 'SHDs', 'TPR', 'FPR', 'BIC']])
    print(lambdas[np.argmin(results_df.BIC)] / np.log(6))

    # results_df.to_csv("./alpha_results/MISOCP_alpha_results_15.csv", index=False)
