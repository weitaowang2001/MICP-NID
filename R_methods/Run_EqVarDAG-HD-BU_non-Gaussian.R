###############################################
# Run EqVarDAG-HD-BU on all NID datasets with non-Gaussian errors
###############################################
...
source("/Users/tongxu/Downloads/projects/micodag/R_methods/EqVarDAG_HD_CLIME.R")


setwd("/Users/tongxu/Downloads/projects/micodag")
dataset.folder <- "/Users/tongxu/Downloads/projects/micodag/Data/RealWorldDatasetsTXu-NonGaussian_30/"
...
for (dataset in datasets) {
  ...
  for (kk in c(1:30)) {
    ...
    result <- list(network=dataset,k=kk, Time=TIME, d_cpdag=d_cpdag, SHD=SHD, SHDs=SHDs, TPR=rates$TPR, FPR=rates$FPR)
    print(result)
...
write.csv(results, "./experiment results/comparison to other benchmarks with non-Gaussian errors/1-30/EqVarDAG-HD-BU_results_non-Gaussian_1_30.csv",row.names=FALSE)
