###############################################
# Run pc algorithm on all NID datasets with estimated moral graph
###############################################
...
for (dataset in datasets) {
  ...
  for (kk in c(1:30)) {
    ...
    start_time <- Sys.time()
    pc.fit = pc(suffStat = list(C=cor(X), n =500), indepTest = gaussCItest, alpha=0.01, labels = colnames(X), fixedGaps = est.moral.graph)
    end_time <- Sys.time()
...
write.csv(results, "./experiment results/comparison to other benchmarks/1-30/pc_results_est_1_30.csv",row.names=FALSE)
