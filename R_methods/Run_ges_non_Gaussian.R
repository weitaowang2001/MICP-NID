###############################################
# Run ges algorithm on all NID datasets with non-Gaussian errors
###############################################
...
for (dataset in datasets) {
  ...
  for (kk in c(1:30)) {
    ...
    result <- list(network=dataset,k=kk, Time=TIME, d_cpdag=d_cpdag, TPR=TPR, FPR=FPR)
    print(result)
    results = rbind(results, result)
  }
}
mean(results$d_cpdag)
write.csv(results, "./experiment results/comparison to other benchmarks with non-Gaussian errors/1-30/ges_results_non-Gaussian.csv",row.names=FALSE)
