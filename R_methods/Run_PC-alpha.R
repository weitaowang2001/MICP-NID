###############################################
# Run PC algorithm on all NID datasets for the alpha experiments, with true moral graph
###############################################
...
dataset.folder <- "/Users/tongxu/Downloads/projects/micodag/Data/SyntheticDataNID/"
...
for (mm in c(10, 15, 20)) {
  ...
  for (alpha in c(1,2,4)) {
    ...
    for (kk in c(1:10)) {
      ...
      result <- list(m=mm, alpha=alpha, instance=kk, time=TIME, d_cpdag=d_cpdag, TPR=TPR, FPR=FPR)
      print(result)
...
write.csv(results, "pc_alpha_results.csv",row.names=FALSE)
